...
Authorization: an OAuth 2.1 access token (mcp.oauth.token) and nothing else.
A person signs in as themselves, through a browser, and the request env is
switched to that user before any tool runs, so ir.model.access + ir.rule
enforce underneath the MCP governance scope. On a missing or invalid token we
answer 401 with a WWW-Authenticate header pointing at the RFC 9728 resource
metadata, which is how MCP clients discover OAuth.
"""
import base64
import binascii
import json
...
SERVER_INSTRUCTIONS = (
    ...
    "resolve names and aggregate, and you cannot create, update or delete "
    "anything. If the user asks for a change, tell them this connector cannot "
    "make it rather than attempting a tool that does not exist. Every call "
    "runs as their own Odoo user and is audited, so you can never see more "
    "than they can. Odoo conventions: filters are domains (lists of "
    "('field','operator',value) triples with '&'/'|' prefix operators); read "
    "with search_records rather than fetching ids then reading them; resolve a "
    "name to an id with name_search before filtering by a related record; "
    "display names come from the model's _rec_name; records are "
    "company-scoped, so results reflect the companies the user has access to; "
    "most business documents start in a draft state and must be confirmed or "
    "posted to take effect; call get_schema before assuming a field exists. "
    "Results are capped - when a reply carries has_more=true, say the answer "
    "is partial and narrow the filter or aggregate with read_group instead of "
    "reporting the page as the whole set."
)
...
class MCPController(http.Controller):
    ...
    def _challenge_scopes(self):
        ...
        `scopes_supported`, so this is the set every connection ends up with.
        Every tool here reads, so read is the whole set.
        """
        return [SCOPE_READ]
    ...
    def health(self, **kw):
        ...
        return self._json({
            ...
            "authMethods": [m for m, on in (
                ("oauth", param_enabled(env, "oauth_enabled", True)),) if on],
        })
