...
class TestOAuth(TransactionCase):
    ...
    def test_scope_normalisation(self):
        self.assertEqual(normalize_scopes("odoo:read"), [SCOPE_READ])
    ...
    def test_write_is_not_a_scope_this_edition_understands(self):
        """A client asking for odoo:write must not be handed it.

        Nothing here can act on a write grant, and minting a token that claims
        one would let a later module honour it without the governance that is
        supposed to come with it.
        """
        self.assertEqual(SCOPES_SUPPORTED, [SCOPE_READ])
        self.assertEqual(normalize_scopes("odoo:read odoo:write"), [SCOPE_READ])
        self.assertEqual(normalize_scopes("odoo:write"), [SCOPE_READ])

    def test_scope_default_is_opt_in_only(self):
        """RFC 6749 §3.3 lets us pick the default when a client sends none.

        It must stay least-privilege, so an empty scope string can never fall
        open to something wider than the caller asked for.
        """
