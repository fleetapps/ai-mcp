...
Every row it adds is read-only, because reading is all this edition does. AI
MCP Governance replaces the flat "add" with a preset — read, read+draft, or
full — once there are write bits for a preset to set.
...
class MCPModelPicker(models.TransientModel):
    ...
    def _default_scope(self):
        ...
        if scope:
            return scope
        # Otherwise the one active scope. Ordered by id so two administrators
        # looking at the same database are offered the same default.
