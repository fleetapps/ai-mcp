...
class TestHttpEndpoints(HttpCase):
    ...
    def setUpClass(cls):
        ...
        base = cls.env["ir.config_parameter"].sudo().get_param(
            "web.base.url", "").rstrip("/")
        client = cls.env["mcp.oauth.client"].create({
            "name": "TEST http client",
            "client_id": "mcpc-http-test",
            "token_endpoint_auth_method": "none",
            "redirect_uris": "https://x.example/cb",
        })
        cls.raw_key = new_secret(prefix="mcpat-", nbytes=32)
        cls.token = cls.env["mcp.oauth.token"].issue(
            cls.raw_key, new_secret(prefix="mcprt-"), {
    ...
    def test_health(self):
        ...
        data = r.json()
        self.assertEqual(data["server"]["name"], "odoo-ai-mcp")
