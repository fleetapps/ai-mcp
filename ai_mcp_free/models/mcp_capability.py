...
capability bundles a set of tools; a governance scope switches whole
capabilities on or off. Downstream modules extend the connector simply by
shipping new mcp.capability / mcp.tool data plus, if needed, a new handler
method on mcp.engine - no controller surgery, upgrade-safe.
...
This edition ships only reading verbs. The ``writes`` machinery below is kept
deliberately, because it is the seam a module adding a mutating verb has to
plug into - see ``_write_handlers``.
"""
...
# Empty here: every verb this module implements only reads. The set exists so
# that a module adding a mutating verb has somewhere to declare it, and so the
# engine's guard has something to test against rather than assuming.
WRITE_HANDLERS = set()
...
class MCPTool(models.Model):
    ...
    writes = fields.Boolean(
        ...
        help="Mutating tools are refused unless a module has explicitly "
             "declared the verb as writing and the scope permits it.")
    ...
    def _write_handlers(self):
        ...
        this method, or its tool is treated as a read and executes without any
        of the checks a mutating call is supposed to pass. That is the one way
        a downstream module can silently punch through the governance layer, so
        ``tests/test_permissions.py`` fails if any registered handler is
        classified neither here nor in the read set.
        """
