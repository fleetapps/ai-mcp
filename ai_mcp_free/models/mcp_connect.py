...
class MCPConnect(models.TransientModel):
    ...
    def get_state(self, with_qr=True):
        ...
        stays True so ``fix_base_url``/``revoke``/``add_suggested_models`` and friends,
    ...
    def _connection_status(self):
        ...
        return {
            ...
            "detail": self._status_detail(users, last),
        }

    @api.model
    def _status_detail(self, users, last):
        bits = []
        if users:
    ...
    def _upgrade_state(self):
        """What this edition deliberately does not do, said once and plainly.

        Kept honest on purpose. A free connector that hides its ceiling wastes
        the evaluator's afternoon; one that nags on every screen is worse. So
        this is a single card, at the bottom, that names the three things
        people actually hit - writes, per-team permissions, and long-term audit
        - rather than a generic upsell, and it is hidden entirely from users
    ...
    def add_suggested_models(self):
        ...
        scope = self.env.user.sudo().mcp_effective_scope()
        if scope:
            added = scope.add_models(SUGGESTED_MODELS)
    ...
    def _starter_prompts(self):
        ...
        return [{"text": text, "ask": self._ask_links(text)}
                ...
                if model is None
                or (model in readable and self._user_can_read(model))]
